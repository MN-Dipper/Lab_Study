...
DOFBOT_CONFIG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        ...
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False, # 重力开关
            max_depenetration_velocity=5.0, # 最大穿透恢复速度
        ),
        articulation_props=sim_utils.ArticulationRootPropertiesCfg(
            enabled_self_collisions=True, # 自碰撞检测
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    sim_dt = sim.get_physics_dt() # -物理时间步长
    sim_time = 0.0 # -模拟时间
    count = 0 # -计数器

    while simulation_app.is_running(): # -当模拟应用程序正在运行时
        # reset
        if count % 500 == 0:
            # reset counters
            count = 0
            # reset the scene entities to their initial positions offset by the environment origins
            root_jetbot_state = scene["Jetbot"].data.default_root_state.clone() # -克隆Jetbot的默认根状态
            root_jetbot_state[:, :3] += scene.env_origins # -将环境原点添加到Jetbot的根位置
            root_dofbot_state = scene["Dofbot"].data.default_root_state.clone() # -克隆Dofbot的默认根状态
            root_dofbot_state[:, :3] += scene.env_origins # -将环境原点添加到Dofbot的根位置
        ...
        scene["Jetbot"].set_joint_velocity_target(action) # -把预设的关节速度目标设置给Jetbot

        # wave
        wave_action = scene["Dofbot"].data.default_joint_pos
        wave_action[:, 0:4] = 0.25 * np.sin(2 * np.pi * 0.5 * sim_time)
        scene["Dofbot"].set_joint_position_target(wave_action) # -把预设的关节位置目标设置给Dofbot
